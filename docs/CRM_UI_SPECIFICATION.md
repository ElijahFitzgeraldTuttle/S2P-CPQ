...
The CPQ and CRM systems use separate identifier namespaces with clear ownership:

| Identifier | Owner | Format | Description |
|------------|-------|--------|-------------|
| **Lead/Project ID** | CRM | Integer | Originates from CRM. Passed to CPQ via `leadId` URL param or API field. Links quotes back to CRM records. |
| **Quote ID** | CPQ | Integer | Database primary key, auto-generated by CPQ. Internal reference only. |
| **Quote Number** | CPQ | String | Human-readable format (e.g., "Q-2026-0001"). Generated by CPQ for external reference on proposals/invoices. |

### CRM → CPQ Flow

When opening CPQ from CRM, pass the CRM lead/project ID:

```
/calculator?leadId=12345&company=Acme%20Corp&project=Main%20Office
```

The CPQ stores `leadId` and includes it in:
- All saved quotes (for bi-directional sync)
- API responses back to CRM
- Export documents for traceability

### CPQ → CRM Sync

When syncing quotes back to CRM, the CPQ includes:

```json
{
  "leadId": 12345,           // CRM's identifier (owner: CRM)
  "quoteId": 789,            // CPQ's internal ID (owner: CPQ)
  "quoteNumber": "Q-2026-0045", // CPQ's display number (owner: CPQ)
  "pricing": { ... }
}
```
